...
Currently available only on trunk, this script will need to be fetched explicitly if you are on a 
...
This script has been around for since 0.96, so all mines should have a copy. It is located at
'''intermine/scripts/compare_tempates_for_releases'''. If you do not have a check-out, this file 
...
AlleleClass_Allele                                release-beta:    2132, release-28.0:    2117, diff:   1%
 ...
 1. '''Only in''' - template was found in one mine and not the other.
 1. '''In Both: Diff < 10%''' - template run in both mines and results returned were different.  It's probably safe to assume these are okay.
